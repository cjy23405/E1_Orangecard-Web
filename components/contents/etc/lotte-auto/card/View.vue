...
const apiCarSearch = useApiCarSearch(uid);
...
// state
const card = ref<[string, string, string, string]>([card1, card2, "", ""]);
const agree = ref("");
const devName = ref("");
...
const buttonDisabled = computed(() => {
  return (
    ...
    apiCarSearch.isPending.value ||
    apiCarEdit.isPending.value
  );
});
const myID = computed(() => {
...
const formSchema = toTypedSchema(
  z.object({
    ...
    carNum: z.string().superRefine((val, ctx) => {
      if (!val) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: useT("formMessage.004", [labels.carNum]),
        });
      }
    }),
...
const checkCarCertify = async () => {
  ...
  await apiCarSearch
    .mutateAsync({
      carNum: formValues.carNum,
      name: devName.value || stores.auth.loginInfo?.name || "",
    })
    .catch(() => {
      if (apiCarSearch.error.value) {
        alertOpen({
          message: useApiGetErrorMessage(apiCarSearch.error.value.data),
        });
      }
    });

  if (apiCarSearch.data.value?.data) {
    return true;
  } else {
    return false;
  }
...
// 차량 등록
const addCar = async () => {
  const data = apiCarSearch.data.value?.data;
  ...
  await apiCarEdit.mutateAsync(addData).catch(() => {
    if (apiCarEdit.error.value) {
      alertOpen({
        message: useApiGetErrorMessage(apiCarEdit.error.value.data),
      });
    }
  });

  if (apiCarEdit.isSuccess.value) {
    return true;
  } else {
    return false;
  }
...
const submitCall = useDebounceFn(async () => {
  ...
  const checkCar = await checkCarCertify();
  ...
  const addCarOk = await addCar();

  if (!addCarOk) return;

  router.push(localePath("/etc/lotte-auto/done"));
}, 300);
    ...
    >
      <form ref="formRaw" @submit.prevent="submit">
        <div class="px-8 py-16">
          <FormField name="cardNum">
            <FormItem>
              <FormValidation for="cardNum" :label="labels.cardNum">
                <FormControl>
                  <FormCardInput
                    :id="['cardNum1', 'cardNum2', 'cardNum', 'cardNum4']"
                    v-model="card"
          ...
          <FormField v-slot="{ componentField }" name="carNum">
            <FormItem>
              <FormValidation for="carNum" :label="labels.carNum">
                <FormControl>
                  <div v-if="isDev" class="relative">
                    <input
                      v-model="devName"
                      type="text"
                      class="absolute right-0 top-[-2.4rem] w-40 border-2 border-solid border-red-600 text-center md:top-[-2.7rem]"
                      title="명의자 이름"
                    />
                  </div>
                  <FormBasicInput
